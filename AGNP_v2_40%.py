...
def train():
    ...
    full_adj_set, sub_adj_set, features, targets = load_data_agnp(40)
    ...
    for epoch in range(args.epochs):
        ...
        graph_encoder.train()
        anp_model.train()
        adj_context = mask_graph(sub_adj_set[time_index:time_index + 12], 40)
        ...
        if (epoch + 1) % 1000 == 0:
            ...
            with torch.no_grad():
                ...
                print(mae_list)
                print(mse_list)
                torch.save(graph_encoder.state_dict(), 'logs/epoch_{}_graph_40%.pth'.format(epoch))
                torch.save(anp_model.state_dict(), 'logs/epoch_{}_anp_40%.pth'.format(epoch))
...
def test():
    ...
    graph_encoder = Graph_Encoder(feat_dim, args.hiddenEnc, args.dropout).to(args.device)
    anp_model = Anp(128, [256, 256], 256).to(args.device)
    state = torch.load('logs/epoch_graph_40%.pth', map_location='cuda')
    graph_encoder.load_state_dict(state)
    state = torch.load('logs/epoch_anp_40%.pth', map_location='cuda')
